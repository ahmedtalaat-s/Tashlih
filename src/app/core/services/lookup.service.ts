...
export class LookupService {
  private readonly baseUrl = API_CONSTSANTS.BASE_URL;
  ...
  getVehicleTypes(): Observable<VehicleTypeResponse> {
    return this.http.get<VehicleTypeResponse>(
      `${this.baseUrl}${API_CONSTSANTS.END_POINTS.LOOKUPS.VEHICLE_TYPES}`,
    );
  }
  ...
  getSubcategories(): Observable<SubcategoryResponse> {
    return this.http.get<SubcategoryResponse>(
      `${this.baseUrl}${API_CONSTSANTS.END_POINTS.LOOKUPS.SUBCATEGORIES}`,
    );
  }

  getSubcategoriesByVehicleType(vehicleTypeId: number | string): Observable<any> {
    return this.http.get(
      `${this.baseUrl}${API_CONSTSANTS.END_POINTS.LOOKUPS.VEHICLE_TYPE_SUBCATEGORIES(vehicleTypeId)}`,
    );
  }

  // ===== Makes =====
  getMakes(): Observable<MakeResponse> {
    return this.http.get<MakeResponse>(`${this.baseUrl}${API_CONSTSANTS.END_POINTS.LOOKUPS.MAKES}`);
  }

  getMakesByVehicleType(vehicleTypeId: number | string): Observable<any> {
    return this.http.get(
      `${this.baseUrl}${API_CONSTSANTS.END_POINTS.LOOKUPS.VEHICLE_TYPE_MAKES(vehicleTypeId)}`,
    );
  }
  ...
  getModels(): Observable<VehicleModelResponse> {
    return this.http.get<VehicleModelResponse>(
      `${this.baseUrl}${API_CONSTSANTS.END_POINTS.LOOKUPS.MODELS}`,
    );
  }

  getModelsByMake(makeId: number | string): Observable<any> {
    return this.http.get(`${this.baseUrl}${API_CONSTSANTS.END_POINTS.LOOKUPS.MAKE_MODELS(makeId)}`);
  }

  // ===== Years =====
  getYears(): Observable<YearResponse> {
    return this.http.get<YearResponse>(`${this.baseUrl}${API_CONSTSANTS.END_POINTS.LOOKUPS.YEARS}`);
  }

  // ===== Cities =====
  getCities(): Observable<CityResponse> {
    return this.http.get<CityResponse>(
      `${this.baseUrl}${API_CONSTSANTS.END_POINTS.LOOKUPS.CITIES}`,
    );
  }
  ...
  getPartConditions(): Observable<PartConditionResponse> {
    return this.http.get<PartConditionResponse>(
      `${this.baseUrl}${API_CONSTSANTS.END_POINTS.LOOKUPS.PART_CONDITIONS}`,
    );
  }
  ...
  getWarrantyTypes(): Observable<WarrantyTypeResponse> {
    return this.http.get<WarrantyTypeResponse>(
      `${this.baseUrl}${API_CONSTSANTS.END_POINTS.LOOKUPS.WARRANTY_TYPES}`,
    );
  }
}
